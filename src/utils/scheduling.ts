...
export function validateSched(s: Sched): Record<string, string> {
  ...
  if (s.start && s.end && s.start >= s.end) { errs.end = 'Must be after course start date'; }
  if (s.enrollStart && s.enrollEnd && s.enrollStart >= s.enrollEnd) { errs.enrollEnd = 'Must be after enrollment start'; }
  if (s.start && s.enrollStart && s.enrollStart > s.start) { errs.enrollStart = 'Enrollment must open on or before course start'; }
  if (s.end && s.enrollEnd && s.enrollEnd > s.end) { errs.enrollEnd = 'Enrollment must close on or before course end'; }
  return errs;
}
