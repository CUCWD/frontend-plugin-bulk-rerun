...
const Stepper = ({ steps, cur }) => (
  <div style={{
    display: 'flex', alignItems: 'center', margin: '24px 0px', padding: '12px 24px', background: WHITE,
  }}
  >
    {steps.map((s, i) => {
      const done = i < cur;
      ...
      return (
        <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < steps.length - 1 ? 1 : 'none' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{
              fontSize: 16, fontWeight: active ? 600 : 400, whiteSpace: 'nowrap', color: stepColor(done, active),
            }}
            >
              {s}
            </span>
          ...
          {i < steps.length - 1 && (
          <div style={{
            flex: 1, height: 2, background: done ? SUCCESS : '#e0e0e0', margin: '0 8px', transition: 'background .3s',
          }}
          />
          )}
        </div>
...
const BulkRerunsTabInner = () => {
  ...
  const defaultCfg = {
    ...
    runId: '',
    sched: {
      start: '', end: '', enrollStart: '', enrollEnd: '', pacing: 'instructor',
    },
    certs: {
      mode: 'honor', display: 'early_no_info', create: true, studentGenCert: true, certOnDashboard: true,
    },
    orgRosters: {},
    removeOp: false,
    gating: { mode: 'disabled', templateId: '' },
    fromMode,
    prog,
    newOrgs,
  ...
  return (
    <div style={{ fontFamily: FONT, padding: 24 }}>
      ...
      {/* ── Bulk reruns sub-nav ── */}
      <div style={{
        display: 'flex', marginBottom: 20, borderBottom: `1px solid ${ BORDER}`, background: WHITE, borderRadius: '4px 4px 0 0', padding: '0 4px',
      }}
      >
        {BULK_VIEWS.map(v => {
          ...
          return (
            <div
              ...
              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { setBulkView(v.id); } }}
              style={{
                padding: '11px 18px',
                cursor: 'pointer',
                userSelect: 'none',
      ...
      {bulkView === 'tracking' && (
        <div>
          {/* Sub-tab bar */}
          <div style={{
            display: 'flex', marginBottom: 16, borderBottom: `1px solid ${ BORDER}`, background: WHITE, borderRadius: '4px 4px 0 0', padding: '0 4px',
          }}
          >
            {[
              ...
              return (
                <div
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' || e.key === ' ') { setTrackingSubTab(t.id); setViewingEntry(null); }
                  }}
                  style={{
                    padding: '10px 16px',
                    cursor: 'pointer',
                ...
                <div>
                  ...
                  >
                    <button
                      ...
                      style={{
                        padding: '5px 12px', fontSize: 13, border: `1px solid ${ BORDER}`, borderRadius: 4, background: WHITE, cursor: 'pointer', color: BRAND,
                      }}
                    >
                      History
                    </button>
